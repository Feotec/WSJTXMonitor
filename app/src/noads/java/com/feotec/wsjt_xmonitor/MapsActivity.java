...
public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    /*  This is left over from the attempts to shutdown from timeout or WSJT-X.  There is presently no way doingShutdown will ever return true.
    @Override
    protected void onResume() {
        super.onResume();
        logInfo("onResume() *********************");

        //  If UDPService invoked a shutdown ...
        boolean doingShutdown = WSJTXUtils.getIsFinishInvoked( this );
        if (doingShutdown) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                logInfo("MapsActivity.onResume() finishAndRemoveTask() *****");
                this.finishAndRemoveTask();
            } else {
                logInfo("MapsActivity.onResume() finishAffinity() *****");
                this.finishAffinity();
            }
        }
    }
